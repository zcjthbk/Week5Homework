...
```{r q01}
#Linear mixed effects model with a fixed effect for age, and random effects for sex and participant (partid)
```
...
ggplot(self, aes(x= age, y = riskSR, color = sex))+
...
```{r q05}
#The graph suggests that self-reported risk-taking decreases with age, and that while women seemingly have a lower baseline level of risk-taking, male and female reports converge over time. Suggests a negative correlation between random intercepts and slopes for the random effect "sex".
```
...
```{r q07}
mod2 <- lmer(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3) + (1 | sex), data=self)
summary(mod2)

anova(mod2, mod1) #yes, adding a random effect of sex improves the model
```
...
```{r q09}
props <- combi %>% count(trialtype, RiskyChoice) %>% mutate(proportion = n/sum(n))
...
```{r q10}
#It appears that participants tended to make riskier choices when it was generally "safer" to do so. In other words, when the prospective loss was greater, participants tended to play it safe. When taking a greater risk had less of a possible loss, they were more likely to take the risk.
```
...
```{r q11}
#Given that we're looking at risk, we should consider both participant and self-reported risk as a random effect. It would also be interesting to look at sex as a random effect, as the earlier plot demonstrated some sort of gradual convergence pattern. 
```
