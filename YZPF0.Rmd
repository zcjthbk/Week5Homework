...
```{r q01}
#Linear regression with age as the DV. There are no random effects. 
```
...
```{r q03}
ggplot(self, aes(x=age_cs, y=riskSR, colour=as.factor(sex))) + geom_point() + geom_smooth(method=lm)
```
...
```{r q04}
ggplot(self, aes(x=age_cs, y=riskSR, colour=as.factor(sex))) + 
  geom_point() + 
...
```{r q05}
#Self reported risk attitudes for males are initially higher than females at a young age, but as age increases, male risk attitudes decrease to the same average risk attitudes as females. 
```
...
#Yes, adding sex significantly improves the model fit
```
...
Q9. Overall, on what proportion of trials did people make the riskier choice under the 'normal' and 'high risk' conditions
```{r q09}
combi %>% group_by(trialtype) %>% count(RiskyChoice) %>% mutate(prob=n/sum(n))
```

Q10. How do you interpret this in words?
```{r q10}
#In the normal risk condition, people made a risker choice in 48% of the trials. In the high risk condition, people made a risker choice in 38.7% of the trials. This means when there is a choice with a higher risk, people are less likely to take that risk than a lower risk choice. 
```
