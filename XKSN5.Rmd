...
```{r q01}
#we need a polynomial regression model, with the DV being reported risk attitudes and the predictor being age
...
self<-self%>%mutate(sex_r=if_else(sex==0, 'Male', 'Female'))
ggplot(self, 
       aes(x = age_cs, 
           y = riskSR,
         colour = sex_r)) +
...
```{r q05}
#risk attitudes for men and women converge as they age
```
...
#using lm instead of lmer because we have no random effects
```
...
#yes, adding the effect of sex improves the model
```
...
```{r q09}
combi%>%group_by(trialtype)%>%summarise(proportion=mean(RiskyChoice))
#in the normal trials, participants made a risky decision 48% of the time. In high risk trials, participants made risky trials 38.6% of the time.
```

Q10. How do you interpret this in words?
```{r q10}
#as risk of the situation rises, people are less likely to make risky choices
```
