...
```{r q01}
# A linear effect model is needed. DV is the self-reported risk attitude. Random effect is the individual difference (i.e., participant id).
```
...
```{r q05}
#It suggests that for males, there is no correlation between age and risk attitudes; for females, there is a weak, negative correlation between age and risk attitudes.
```
...
```{r q07}
mod2 <- lm(riskSR ~ sex + age_cs + I(age_cs^2) + I(age_cs^3), 
             data=self)
summary(mod2)
#Yes, as mod2 explains more variance than mod1 (2.19% vs. 0.71%).
```
...
```{r q09}
combi %>% count(trialtype, RiskyChoice)
3451/(3451+3742) #47.98% people made riskier choice under normal conditions
2789/(2789+4419) #38.69% people made riskier choice under high risk conditions.
```

Q10. How do you interpret this in words?
```{r q10}
odds_normal <- (1-0.4798)/0.4798 #odds of choosing save choice under normal condition
odds_highrisk <- (1-0.3869)/0.3869 #odds of choosing save choice under high risk condition
odds_highrisk/odds_normal
#when trials are more risky (trialtype=1) the tendency to choose the safer option increases by 1.46 times.
```
