...
setwd("~/Year 3/Advanced Statistics/Week 5")
...
```{r q01}
#Risk SR is the DV and age is the predictor
#random effect could be participant id and maybe gender (although this maybe a fixed effect)
...
self<- self %>% mutate(age_cs = scale(age, center = TRUE, scale = TRUE))
...
```{r q03}
#it has to be a factor or non numeric?


ggplot(self, aes(x = age, y = riskSR)) + geom_point() + geom_smooth(aes(group = sex),method = lm)
...
```{r q04}
ggplot(self, aes(x = age, y = riskSR)) + geom_point() + geom_smooth(aes(group = sex, color = sex),method = lm)


#the light blue line is female and the dark blue/black line is male
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#Risk taking attitude decreases with age. For women it seems relatively stable and slightly decreases after the age of 60.

#Whereas, men tend to self-report greater risk taking than women until the age of 80 where men and women take risk at a similar level.
...
```{r q06}
#mod1 <- lmer(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3), data=self)
#summary(mod1)

#changed lmer to lm because we are not looking at any random effects
...
#yes because the p-value is significant of this model
```
...
combi<- inner_join(self, gambles, 'partid')

```

Q9. Overall, on what proportion of trials did people made the riskier choice under the 'normal' and 'high risk' conditions
```{r q09}
summary_tbl<- gambles %>% group_by(trialtype) %>% summarise(sum_riskychoice = sum(RiskyChoice), sum_trialtype = sum(trialtype))

#total high risk trials = 7208
#total normal trials = 14401 - 7208 = 7193
...
```{r q10}
#Participants were more likely to make the riskier choice in the normal risk trials (48%) than the high risk trials (38.7%). Thus, they were more likely to choose the safe choise in the high risk trials.
...
```{r q11}
#random effects of participant and scenario
...
mod4 <- lmer(RiskyChoice ~ age_cs + trialtype + (1 | scenario) + (1 + partid), data=combi)
summary(mod4)
