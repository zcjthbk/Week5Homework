...
#DV - self reported risk attitude (self$riskSR), continuous data
#Could include participant (partid) as random effect (sex could be control variable as a fixed effect)
```
...
```{r q02}
self <- mutate(self, age_cs = scale(age)) #should do the same as mutate(self, age_cs = ((age - mean(age)/sd(age))))
sum(self$age_cs) #ensuring that the scaling was successful
head(self) #For some reason, when the dataframe is opened, the age_cs is displayed as age_cs[, 1] although the variable seems to be fine. 
```
...
```{r q03}
ggplot(self %>% mutate(sex = recode(sex, "0" = "male", "1" = "female")),
       aes(x = age, y = riskSR, colour = factor(sex))) +
  geom_point(colour = "black") + #manually change the colour because some of the points cover the regression line and the graph is crowded
  geom_smooth(method = "lm", se = F) + #standard error is removed as the graph is quite crowded
  theme_classic() + 
...
```{r q04}
#I don't understand what the goal here is. The graph above does show what ecah line represents. 
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#Female participants have a fairly stable self reported risk attitude during their lifetime, while for males it decreases with age. There is clear age and sex interaction. Scores seem to converge at around 5.5 - 6, near the middle of the scale. 
```
...
```{r q06}
mod1 <- lm(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3),
             data = self) #change lmer to lm as there are no random effects
summary(mod1)
```

Q7. Does adding an effect of sex improve the model - call this `mod2`? 
```{r q07}
mod2 <- lm(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3) + sex,
             data = self)
anova(mod1, mod2)
#yes, adding sex improves the model
```
...
```{r q09}
combi %>% group_by(trialtype) %>% summarise(prop = sum(RiskyChoice)/nrow(combi))
#Riskier choices were made in the normal condition
```
...
#Note: the text above is not mine - it was already in the file and I agree with the assessment based on the calculations in Q9.
```
...
```{r q11}
#participant (partid), scenario
#sex could be used as fixed effect
```
...
```{r q12}
mod4 <- glmer(RiskyChoice ~ trialtype + (1 + trialtype | partid) + (1 | scenario),
              family = "binomial",
              data = combi) #use glmer as RiskyChoice is binary variable
```
