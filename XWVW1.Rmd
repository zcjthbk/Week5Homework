...
```{r q01}
#linear mixed effect model, DV is self reported risk attitude, random effects could be participant and sex.
```
...
```{r q05}
#females report lower risk taking attitudes
```
...
summary(mod2)
r.squaredGLMM(mod2)
#Yes, with only the fixed effects explains 0.7% of the variance in riskSR, but by adding a random effect of sex, the model now explains 3.7% of the variance.
```
...
```{r q09}
combi%>%group_by(trialtype)%>%summarize(proportion=mean(RiskyChoice)*100)
```

Q10. How do you interpret this in words?
```{r q10}
#In the normal risk condition, people made higher proportion of riskier choices.
```
