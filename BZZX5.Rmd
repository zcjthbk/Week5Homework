...
```{r q03}
ggplot(self, aes(age_cs, riskSR)) + 
  geom_point() +
  geom_smooth(aes(colour = as.factor(sex)), method='lm')
...
```{r q05}
#risk attitudes is not likely to be predicted by age for female. risk attitudes may reduce as age increases for male.
```
...
summary(mod1)
#use lm() when there is no random effects in the model
```
...
summary(mod2)
anova(mod1,mod2)
#no, adding the effect of sex does not imporve the model
```
...
```{r q09}
combi %>% group_by(trialtype) %>% summarise(mean(RiskyChoice))
#normal condition: 48.0% of trials
#high risk condition: 38.6% of trials
```

Q10. How do you interpret this in words?
```{r q10}
#People were less often to make a riskier choice in conditions in which taking the risky option would lead to a loss in the long run.
```
...
```{r q11}
#random effects: participant, scenario
```
...
```{r q12}
#change optimizer so model would converge
mod4 <- lmer(RiskyChoice ~ trialtype + (1 + trialtype | scenario) + (1 | partid), data=combi, control = lmerControl(optimizer="bobyqa"))
summary(mod4)
```
