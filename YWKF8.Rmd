...
```{r q01}
# linear regression. DV is self reported risk. Random effects can be participants and gender.
```
...
```{r q02}
mu_age <- self %>% pull(age) %>% mean()
sd_age <- self %>% pull(age) %>% sd()

self <- self %>% 
  mutate(age_c = age - mu_age) %>% 
  mutate(age_cs = age_c/sd_age)
```
...
```{r q03}
ggplot(self, aes(x = age_cs, y = riskSR, group = as.character(sex))) + 
  geom_point() + 
...
```{r q05}
# risk attitudes are not affected by age.
```
...
```{r q09}
combi %>% group_by(trialtype) %>% summarise(n = n(), mean = mean(RiskyChoice))
```

Q10. How do you interpret this in words?
```{r q10}
# 48% of choosing riskier choice in normal condition while 39% chance people choose riskier choice in high risk condition. The percentage of making riskier choice is lower in high risk condition.
```
...
```{r q12}
mod4 <- glmer(RiskyChoice ~ trialtype + (1 + trialtype | partid) + (1 | scenario), dat = combi, family = binomial)
summary(mod4)
```
