...
```{r q02}
self<-self%>%mutate(age_cs=(age-mean(age))/sd(age))
```
...
```{r q05}
# the age negative predict self-report risk attitude among male, no significant relationship between age and self-report risk attitude among female.
```
...
summary(mod2) # improve as the adjusted R2 increase
```
