...
```{r q01}
#simple linear model
#The DV is the self-reported attitude, and no for random effects
```
...
```{r q02}
self2 <- self %>% mutate(age_cs = age-18)
```
...
```{r q03}
self3 <- self2 %>% mutate(sex = recode(as.character(sex), '0' = 'male', '1' = 'female'))
ggplot(self3, aes(x = age, y = riskSR, colour = factor(sex))) + geom_point() + geom_smooth(method = 'lm')
...
```{r q05}
#The graph shows us that with both genders, as age increases, risk attitudes decreases. Although, the decrease for male is higher. 
```
...
```{r q06}
mod1 <- lmer(riskSR ~ age_cs + I(1 +sex | age_cs^2) + I(1+ sex | age_cs^3), data=self3)
summary(mod1)
```

Q7. Does adding an effect of sex improve the model - call this `mod2`? 
```{r q07}
mod2 <- lmer(riskSR ~ age_cs + sex +  I(1+sex |age_cs^2) + I( 1+sex|age_cs^3), data=self3)
summary(mod2)
```
...
```{r q08}
combi <- inner_join(self3, gambles, by = 'partid')
```

Q9. Overall, on what proportion of trials did people made the riskier choice under the 'normal' and 'high risk' conditions
```{r q09}
table <- combi %>% group_by(trialtype,RiskyChoice) %>% summarise(n = n())

3451/(3451+3742)
#48% in the normal condition chose the risky choice
2789/(2789+4419)
#38.7% in the riskier condition chose the risky choice
```

Q10. How do you interpret this in words?
```{r q10}
#In the high risk condition, people were less willing to pick the risky option than in the normal condition. People were risk averse in the high condition.
```
...
```{r q12}
mod4 <- lmer(scenario~ trialtype + (1+ partid | partid), + (1|scenario), data = combi)
summary(mod4)
```
