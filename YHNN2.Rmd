...
```{r q01}
#DV = riskSR (self-reported risk attitude)
...
```{r q03}
ggplot(self, aes(x=age, y=riskSR))+
  geom_point()+
  geom_smooth(method="lm", aes(colour=as.factor(sex)))
```
...
ggplot(self, aes(x=age, y=riskSR))+
  ...
  geom_smooth(method="lm", 
              aes(colour=as.factor(sex)),
              se=FALSE)+
  scale_colour_discrete(labels=c("Male", "Female"))+
  labs(colour="Sex", y="Self-Assessed Risk", x="Age")+
  theme_minimal()
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#It suggests that there is no association between age and self-assessed risk scores in women,whereas there appears to be a small negative correlation between self-assessed risk scores and age in men. 
```
...
#Yes, adding the sex improves the model. 
```


Q8. Now lets combine the self and gambles data and call this `combi`
```{r q08}
gambles <- unique(gambles) #remove repeated entries
...
#trialtype: 0 = normal trial, 1 = higher risk trial
#RiskyChoice: 0 = safe choice chosen, 1 = risky choice chosen
combi %>% group_by(trialtype) %>% filter(RiskyChoice=="1") %>% summarise(n=n(), percentage=n*100/4205)

#54% risky decisions when the trial was normal
#45% risky decisions when the trial was high-risk
```

Q10. How do you interpret this in words?
```{r q10}
#People are more likely to make risky decisions when these do not affect them on the long term.
```
...
```{r q12}
mod4 <- glmer(RiskyChoice~trialtype+(1+trialtype|partid)+(1|scenario), data=combi, family="binomial")
summary(mod4)
```
