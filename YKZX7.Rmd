...
```{r q01}
#a simple linear model, with dv being risk attitude and iv being age. We could create a mixed effects model with random effects of participant 
```
...
```{r q02}
self <- self %>% mutate(age_c = age - 18) %>% mutate(age_cs = age_c - mean(age_c))
```
...
ggplot(self, aes(x = riskSR, y = age_cs, colour = sex)) +
  geom_point() +
...
```{r q05}
#decline for men and stay relatively stable for women
```
...
```{r q10}
#in the normal conditions participants took the riskier choice 47% of the time and under high risk conditions 38% 
```
...
```{r q11}
#pid because it takes a within subjects approach,  scenario
```
...
summary(mod4)
#Initially, I tried using glmer because I think that RiskyChoice is a categorical variable, but then it gives me an error and asks me to switch to lmer. I was wondering why is that? Do I have to mutate RiskyChoice to be a factor? 
```
