...
```{r q01}
#We can use a linear mixed-effects model; DV is riskSR; random effects include partid
```
...
y = riskSR)) +
geom_point()+
geom_smooth(method=lm, aes(color=as.factor(sex)))
```
...
geom_point()+
geom_smooth(method=lm, aes(color=as.factor(sex)))+
  scale_color_manual(labels = c("male", "female"), values = c("skyblue", "pink"))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#It suggests that as age increases, there is generally lower self-reported risk taking for males, but there is not much change in ratings for females.
```
...
```{r q09}
combi%>% group_by(trialtype) %>% count(RiskyChoice) %>% summarise(prop=n/sum(n))
```
