...
```{r q01}
#To analyse whether self-reported risk attitude (DV: riskSR) is predicted by age (IV: age) we can use a linear mixed effects model. This model allows us to include random effects such as participant ID or sex if desired.
```
...
```{r q02}
self = self %>% mutate(age_cs = as.numeric(scale(age)))
```
...
ggplot(self, aes(x = age, y = riskSR, colour = sex)) + geom_point() + geom_smooth(method = "lm")
```
...
```{r q04}
ggplot(self, aes(x = age, y = riskSR, colour = sex)) + geom_point() + geom_smooth(method = "lm") + scale_colour_manual(name = "Sex", values = c("light blue", "pink"), labels =c("male", "female"))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#The graph suggests that male and female risk attitudes converge over time.
```
...
#Adding sex as a predictor improves the model. 
```


Q8. Now lets combine the self and gambles data and call this `combi`
```{r q08}
combi = inner_join(gambles, self, by = "partid")
```

Q9. Overall, on what proportion of trials did people made the riskier choice under the 'normal' and 'high risk' conditions
```{r q09}
combi %>% group_by(trialtype) %>% summarise(pp.risky.trials = round(mean(RiskyChoice), 2)) %>% mutate("Trial Type" = dplyr::recode(trialtype, "0" = "normal", "1" = "hard")) %>% select("Trial Type", pp.risky.trials)
```

Q10. How do you interpret this in words?
```{r q10}
#I assume the answer was already wrongfully included. See comment below.
...
```{r q11}
#You may want to consider the random effect of participant, scenario, and sex. 
```
