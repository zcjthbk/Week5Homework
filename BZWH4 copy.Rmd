...
```{r q01}
#linear regression
#DV is self reported risk attitude (riskSR)
#no random effects 
```
...
```{r q03}
self <- self %>% mutate(sex=recode(sex, '0'='male', '1'='female'))
...
```{r q05}
#at the age of 18, risk attitudes are higher for males than for females. as age increases, risk attitudes decrease for both genders, but this decrease is greater for males than for females.
```
...
#changed from lmer to lm as there are no random effects
```
...
```{r q09}
combi %>% group_by(trialtype) %>% summarise(prop_r=(1-mean(RiskyChoice)))
#normal: 52.0%, risky: 61.3%
```

Q10. How do you interpret this in words?
```{r q10}
#when trials are more risky (trialtype=1) the tendency to choose the riskier option increases
```
