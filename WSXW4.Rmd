...
```{r q01}
#We need linear mixed model for this. DV is self-reported risk attitude. Random effect is participants.
```
...
```{r q02}
self_c <- self %>% mutate(age_cs=scale(age))
```
...
```{r q03}
self_c <- self_c %>% mutate(sex=as.factor(sex))
ggplot(self_c, aes(x=age, y=riskSR)) + geom_point() + geom_smooth(method=lm, aes(colour=sex))
```
...
```{r q04}
#I think Q3 already made the two regression lines distinguishable?
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#Females keep almost the same risk attitudes across ages, while older males have lower risk-taking attitudes.
```
...
```{r q06}
mod1 <- lm(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3), data=self_c)
summary(mod1)
```

Q7. Does adding an effect of sex improve the model - call this `mod2`? 
```{r q07}
mod2 <- lm(riskSR ~ age_cs + sex + I(age_cs^2) + I(age_cs^3), data=self_c)
anova(mod2, mod1)
#Yes, adding an effect of sex improves the model.
```


Q8. Now lets combine the self and gambles data and call this `combi`
```{r q08}
combi <- inner_join(self_c, gambles, by="partid")
```
...
#Under the normal condition, people made riskier choice in 47.98% of the trials; under the high risk condition, people made riskier choice in 38.69% of the trials.
```

Q10. How do you interpret this in words?
```{r q10}
#People make more risky choices in normal conditions than in high risk conditions.
```
...
```{r q11}
#We should consider the random effects of participants and scenarios.
```
