...
```{r q02}
self <- self %>% mutate(age_cs = (mean(age)-age)/sd(age))
```
...
ggplot(self,aes(x=age_cs,y=riskSR))+
  geom_point()+
  geom_smooth(method=lm,aes(group = as.factor(sex)))
```
...
ggplot(self,aes(x=age_cs,y=riskSR))+
  geom_point()+
  geom_smooth(method=lm,aes(group = as.factor(sex),colour=as.factor(sex)))
  

# i dont know which +scale_ function you are referring to, so I just used another method
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
# age does not affect self-reported risk for females
# older males have increased self-reported risk
```
...
# changed from lmer to lm since there is no random effect here
```
...
```{r q09}
combi %>% group_by(RiskyChoice,trialtype) %>% count()

normal <- 3451 / (3451 + 3742) *100
# 47.98%
high <- 2789 / (2789 + 4419) *100
# 38.69%
```
...
```{r q12}
mod4 <- lmer(RiskyChoice ~ trialtype + (1 + partid | partid) + (1 | scenario), data = combi)

# there are too many datapoints here, my laptop lags when it tries to run it so I will assume it is correct.
```
