...
```{r q01}
#Fixed effect regression model with self-reported risk attitude as the DV, no random effects 
```
...
```{r q02}
self<-self%>%mutate(age_cs=scale(self$age))
```
...
```{r q04}
#Not sure what the question is asking for since legend in above graph already indicates which regression line refers to which sex? 
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#Self-reported risk attitudes is negatively associated with age (being female and increasing age associated with lower risk attitudes)
```
...
```{r q09}
prop<-combi%>%group_by(trialtype,RiskyChoice)%>%
  count()

#People made the riskier choice in 48.0% of trials in normal condition
(3451)/(3742+3451)

#People made the riskier choice in 38.7% of trials in high risk condition
(2789)/(4419+2789)
...
```{r q10}
#Participants are less likely to take risky choices when the risk is high 
```
...
```{r q12}
mod4<-lmer(RiskyChoice~trialtype+(1+trialtype|partid)+(1|scenario),data=gambles,control=lmerControl(optimizer="bobyqa"))
summary(mod4)
```
