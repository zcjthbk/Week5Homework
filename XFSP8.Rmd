...
```{r q02}
self <- mutate(self, age_cs = scale(age))
```
...
```{r q03}
#change sex to binary / categorical
self$sex <- as.factor(self$sex)

ggplot(self ,aes(x=age_cs, y=riskSR, group=sex, color=sex))+
geom_point()+
theme_bw()+
...
scale_color_manual(labels = c("Female", "Male"),
                     values = c("#17A589", "#EB984E"))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#self-reported risk taking decreases with age across both sexes, but to a greater extent in females than males
```
...
#unsure, tried adding (1 + age_cs | partid) but got this error message: Error: number of levels of each grouping factor must be < number of observations (problems: partid)
```
...
```{r q08}
combi <- inner_join(self,gambles)
```

Q9. Overall, on what proportion of trials did people made the riskier choice under the 'normal' and 'high risk' conditions
```{r q09}
combi %>%
  group_by(trialtype,RiskyChoice) %>%
  count()

3451	/ (3742 + 3451 + 4419 + 2789) #0.2396361

#under normal conditions, 24% of participants made the riskier choice

2789	/ (3742 + 3451 + 4419 + 2789) #0.1936671

#under high risk conditions, 19% of participants made the riskier choice
```
...
```{r q12}
mod4 <- lmer(RiskyChoice ~ trialtype + (1 + trialtype || partid) + (1 | partid) + (1 | scenario), data=combi, control = lmerControl(optimizer="bobyqa"))

summary(mod4)

#unsure how to get the model to converge;
#added the "bobyqa" optimizer and tried making random effects uncorrelated with the || sign (as suggested in the practical worksheet)
```
