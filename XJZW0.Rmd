...
```{r q01}
# The DV is self-reported risk attitude (riskSR). There is a random effect of participants.
```
...
```{r q03}
ggplot(self, aes(x = age, y = riskSR, colour=as.factor(sex))) + geom_point() + geom_smooth(method = lm)
```
...
```{r q04}
ggplot(self, aes(x = age, y = riskSR, colour=as.factor(sex))) + geom_point() + geom_smooth(method = lm) + 
  labs(x = "Age", y = "Self reported risk taking") +
...
```{r q05}
#Risk taking score decreases as age increases.
```
...
summary(mod2)
anova(mod1, mod2)
#Adding an effect of sex significantly improves the model.
```
...
```{r q09}
combi %>% filter(trialtype=="0") %>% summarise(sum(RiskyChoice)/n())
#48.0% of people made the riskier choice under normal condition.
combi %>% filter(trialtype=="1") %>% summarise(sum(RiskyChoice)/n())
#38.7% of people made the riskier choice under normal condition.
```
...
```{r q11}
#Ramdom effects of participants and scenarios should be considered.
```
