...
```{r q01}
#Method: GLMM, DV: riskSR, Random Effects: partid, sex
```
...
```{r q02}
self <- mutate(self, age_cs = scale(self$age, center = T, scale = T))
...
```{r q03}
self$sex[self$sex == "0"] <- "Male"
self$sex[self$sex == "1"] <- "Female"
...
grouped_ggscatterstats(
  ...
  y                = riskSR,
  grouping.var     = sex,
  xlab             = "age (centred scaled)",
  ggtheme          = ggplot2::theme_grey(),
  plotgrid.args    = list(nrow = 1),
  annotation.args  = list(title = "risk - age by sex")
)
#binominal 
```
...
```{r q04}
ggplot(self, aes(x=age_cs, y=riskSR, col = sex,
           group = sex)) +
  geom_point() +
...
```{r q05}
#female has higher risk attitude than man in youger age but it showed a decsing trend that at old age they have same level of risk attitude as men
```
...
```{r q07}
mod2 <- lm(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3) + (1 + age_cs | as.logical(sex)), data = self)
summary(mod2)
```
...
```{r q08}
combi <- merge(x = gambles, y =self, by="partid", all.x = TRUE)
```

Q9. Overall, on what proportion of trials did people made the riskier choice under the 'normal' and 'high risk' conditions
```{r q09}
overall <- combi %>% group_by(trialtype, RiskyChoice) %>% summarise(poportion = length(RiskyChoice)/14401)
```
...
high=0.1936671/(0.1936671+0.3068537)
low=0.2396361/(0.2396361+0.2598431)
#by shifting trial type from normal (0) to risky (1) the chance of observing 1 in RiskyChoice goes up by 9.3%
```
...
```{r q12}
mod4 <- glmer(riskSR ~ trialtype + I(trialtype^2) + I(trialtype^3) + (1+trialtype|partid) + (1|partid) + (1|scenario), data = combi)
summary(mod4)
```
...
Hit the green code butto n and copy the https code to clone it.
