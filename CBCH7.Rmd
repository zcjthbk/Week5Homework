...
#DV is self-reported attitude
#random effect of participant and sex
```
...
```{r q02}
self<-self%>%mutate(age_cs=scale(age, center = T, scale = T))
```
...
ggplot(self, aes(x=age, y=riskSR, colour=sex))+
...
```{r q04}
self2<-self%>%mutate(sex=as.factor(sex))
ggplot(self, aes(x=age, y=riskSR, colour=sex))+
  geom_point(alpha=0.2)+
  geom_smooth(method=lm)+
  scale_color_manual(labels=c("Female", "Male"), values=c("darkred", "navy"))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#At young age risk-taking is higher for males than females, but with age it decreases for both and eventually equates around the age of 80
```
...
```{r q08}
combi<-inner_join(self, gambles, by="partid")%>%unique()
```

Q9. Overall, on what proportion of trials did people made the riskier choice under the 'normal' and 'high risk' conditions
```{r q09}
combi%>%group_by(trialtype)%>%summarise(p=mean(RiskyChoice))
#normal condition: 48.44%
#risky condition:41.25%
#first unique() rows needs to be taken from combi, otherwise biased results, cause some rows are repeated more times than others
```

Q10. How do you interpret this in words?
```{r q10}
#participants made more risky choices in normal condition
```
...
```{r q11}
#self reported risk, sex and participant
```
...
```{r q12}
combi<-combi%>%mutate(partid=as.factor(partid), scenario=as.factor(scenario))%>%drop_na()
mod4<-glmer(RiskyChoice ~ trialtype + (1|scenario) +(1+trialtype|partid), data=combi, family='binomial')
summary(mod4)
```
