...
```{r q01}
#linear mixed effect model, dv is self-reported risk attitude, the random effect is participant, sex
```

Q2. First we will scale the age variable in the `self` dataframe and call it `age_cs` Note: this is what they did in the original study (unlike previous times when we have merely centrered it).
```{r q02}
...
```{r q03}
ggplot(self, aes(x = age, y = riskSR, colour = factor(sex))) + geom_point() + geom_smooth(method = "lm")
#sex needs to be categorical 
```
...
```{r q04}
ggplot(self, aes(x = age, y = riskSR, colour = factor(sex))) + geom_point() + geom_smooth(method = "lm") + scale_x_continuous(limits=c(18,90))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#As age increases, the risk attitudes decreases for women 
```
...
summary(mod2)
anova(mod1, mod2) #yes it imrpoves
```


Q8. Now lets combine the self and gambles data and call this `combi`
```{r q08}
combi <- inner_join(gambles, self)
```

Q9. Overall, on what proportion of trials did people made the riskier choice under the 'normal' and 'high risk' conditions
```{r q09}
gambles %>% group_by(trialtype, RiskyChoice) %>% summarise(n =n())
3451/(3451+3742) #48.0% for people to make risky choice in normal condition
2781/(2781+4419) #38.6% for people to make risky choice in high risk conditions 
```

Q10. How do you interpret this in words?
```{r q10}
# people are more likely to take risks in normal condition than when they are in a high risk situation
```
...
```{r q12}
mod4 <- glmer(RiskyChoice ~ trialtype + age + (1|scenario) + (1 + age|partid), data = combi, family = 'binomial', control = lmerControl(optimizer="bobyqa"))
summary(mod4)
```
