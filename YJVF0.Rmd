...
```{r q01}
#We need a linear mixed effects model. The DV is self-reported risk attitude. There is a random effect of participant.
```

Q2. First we will scale the age variable in the `self` dataframe and call it `age_cs` Note: this is what they did in the original study (unlike previous times when we have merely centered it).
```{r q02}
...
ggplot(self, aes(x=age_cs, riskSR, color=sex)) +
  geom_point(color="Black") +
  geom_smooth(method="lm") +
  labs(x="Age (Centered)", y="Self-reported risk taking", colour="Sex") +
  theme_bw()
```
...
ggplot(self, aes(x=age_cs, riskSR, color=sex)) +
  geom_point(color="Black") +
  geom_smooth(method="lm") +
  labs(x="Age (Centred)", y="Self-reported risk taking", colour="Sex") +
...
```{r q05}
#The graph suggests that self-reported risk taking tends to be stable across age for females, but decreases slightly for males.
```
...
```{r q06}
mod1 <- lmer(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3) + (1|partid), , control = lmerControl(check.nobs.vs.nlev="ignore", check.nobs.vs.nRE="ignore"), data=self)
summary(mod1)
```

Q7. Does adding an effect of sex improve the model - call this `mod2`? 
```{r q07}
mod2 <- lmer(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3) + sex + (1|partid), , control = lmerControl(check.nobs.vs.nlev="ignore", check.nobs.vs.nRE="ignore"), data=self)
summary(mod2)
anova(mod1, mod2)
#Yes, adding an effect of sex improves the model, Chisq(1)=14.56, p<.001.
```
...
```{r q09}
combi %>% group_by(trialtype) %>% summarise(mean=mean(RiskyChoice))
#Under the normal conditions, people made the riskier choice for 48% of trials. Under high risk conditions, people made the riskier choice for 39% of trials.
```

Q10. How do you interpret this in words?
```{r q10}
#Under the high risk conditions, people tend make safer choices compared to the normal conditions.
```
...
```{r q11}
#We should consider the random effects of participant and scenario.
```
...
```{r q12}
mod4 <- lmer(RiskyChoice~trialtype+(1+trialtype|partid)+(1|scenario), data=gambles)
summary(mod4)
```
