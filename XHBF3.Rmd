...
```{r q01}
# We need to use a linear mixed model. 
# the dependent variable is riskSR. 
# The random effects are trial type, scenario and sex. 
...
```{r q03}
ggplot(data = self, aes(x = age, y = riskSR, color = as.factor(sex))) +
  geom_point()+ 
...
```{r q05}
# there is no correlation between age and self reported risk attitude for women.  
# There is a weak negative correlation between age and self reported risk attitude for men. 
```
...
```{r q06}
mod1 <- glm(riskSR ~ age_cs + I(age_cs^2) + I(age_cs^3) + (1+ age_cs|partid), data=self)
summary(mod1)
...
```{r q07}
mod2 <- lmer(riskSR ~ age_cs + sex +  I(age_cs^2) + I(age_cs^3) + (1|sex), data=self)
tab_model(mod1, mod2)
# no
```
...
```{r q09}
# normal
norm <- combi %>% filter(trialtype == 0) %>% count(RiskyChoice) %>% mutate(percent = n/sum(n),
                                                                   cond = "norm")

# high risk

high_risk <- combi %>% filter(trialtype == 1) %>% count(RiskyChoice) %>% mutate(percent = n/sum(n),
                                                                   cond = "high risk") 

rbind(norm, high_risk) 

# the proportion of risky choices in the normal condition is .48.

# the proportion of risky choices in the high risk condition is .37
```
...
```{r q10}
# participants are more likely to choose the safe choice when the risk is high compared to normal. 
```
...
```{r q12}
mod4 <- glmer(data = combi, RiskyChoice ~ trialtype + (1 + trialtype|partid) + (1|scenario))
summary(mod4)
```
