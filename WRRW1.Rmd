...
#DV: riskSR
#partid and sex can be the random effects
```
...
```{r q02}
self<-self %>% mutate(age_cs = scale(age_cs))
```
...
ggplot(self, aes(age, riskSR, colour = sex)) +
  geom_point() +
...
```{r q05}
#as age increases, self-reported risk attitude decreases, with male's risk attitude more influenced by age than female's
```
...
```{r q09}
combi %>% group_by(trialtype) %>%
  summarise(p_risky = mean(RiskyChoice))
```

Q10. How do you interpret this in words?
```{r q10}
#people are more risk-averse under risky condition than under normal condition
```
...
```{r q11}
#partid, sex, and scenario
#reason for sex: e.g., males are more risk-seeking than females? but maybe this belongs to fixed-effect?
```
...
```{r q12}
mod4<-glmer(RiskyChoice~trialtype + age + (1 + trialtype + age | partid) + (1|scenario), combi, family = "binomial")
```
